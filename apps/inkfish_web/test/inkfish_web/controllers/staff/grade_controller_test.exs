defmodule InkfishWeb.Staff.GradeControllerTest do
  ...
  describe "index" do
    test "lists all grades", %{conn: conn} do
      conn = get(conn, Routes.staff_grade_path(conn, :index))
  ...
  describe "new grade" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.staff_grade_path(conn, :new))
  ...
  describe "create grade" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.staff_grade_path(conn, :create), grade: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.staff_grade_path(conn, :show, id)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.staff_grade_path(conn, :create), grade: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Grade"
    end
  end
  ...
  describe "update grade" do
    ...
    test "redirects when data is valid", %{conn: conn, grade: grade} do
      conn = put(conn, Routes.staff_grade_path(conn, :update, grade), grade: @update_attrs)
      assert redirected_to(conn) == Routes.staff_grade_path(conn, :show, grade)
    ...
    test "renders errors when data is invalid", %{conn: conn, grade: grade} do
      conn = put(conn, Routes.staff_grade_path(conn, :update, grade), grade: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Grade"
    end
  end
  ...
  describe "delete grade" do
    ...
    test "deletes chosen grade", %{conn: conn, grade: grade} do
      conn = delete(conn, Routes.staff_grade_path(conn, :delete, grade))
      assert redirected_to(conn) == Routes.staff_grade_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.staff_grade_path(conn, :show, grade))
      end
    end
  end
