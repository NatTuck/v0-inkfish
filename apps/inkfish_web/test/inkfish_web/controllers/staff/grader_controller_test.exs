defmodule InkfishWeb.Staff.GraderControllerTest do
  ...
  describe "index" do
    test "lists all graders", %{conn: conn} do
      conn = get(conn, Routes.staff_grader_path(conn, :index))
  ...
  describe "new grader" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.staff_grader_path(conn, :new))
  ...
  describe "create grader" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.staff_grader_path(conn, :create), grader: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.staff_grader_path(conn, :show, id)

      conn = get(conn, Routes.staff_grader_path(conn, :show, id))
      assert html_response(conn, 200) =~ "Show Grader"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.staff_grader_path(conn, :create), grader: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Grader"
    end
  end
  ...
  describe "edit grader" do
    ...
    test "renders form for editing chosen grader", %{conn: conn, grader: grader} do
      conn = get(conn, Routes.staff_grader_path(conn, :edit, grader))
  ...
  describe "update grader" do
    ...
    test "redirects when data is valid", %{conn: conn, grader: grader} do
      conn = put(conn, Routes.staff_grader_path(conn, :update, grader), grader: @update_attrs)
      assert redirected_to(conn) == Routes.staff_grader_path(conn, :show, grader)

      conn = get(conn, Routes.staff_grader_path(conn, :show, grader))
      assert html_response(conn, 200) =~ "some updated kind"
    end

    test "renders errors when data is invalid", %{conn: conn, grader: grader} do
      conn = put(conn, Routes.staff_grader_path(conn, :update, grader), grader: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Grader"
    end
  end
  ...
  describe "delete grader" do
    ...
    test "deletes chosen grader", %{conn: conn, grader: grader} do
      conn = delete(conn, Routes.staff_grader_path(conn, :delete, grader))
      assert redirected_to(conn) == Routes.staff_grader_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.staff_grader_path(conn, :show, grader))
      end
    end
  end
