defmodule InkfishWeb.SubControllerTest do
  ...
  describe "index" do
    test "lists all subs", %{conn: conn} do
      conn = get(conn, Routes.sub_path(conn, :index))
  ...
  describe "new sub" do
    test "renders form", %{conn: conn} do
      conn = get(conn, Routes.sub_path(conn, :new))
  ...
  describe "create sub" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, Routes.sub_path(conn, :create), sub: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, Routes.sub_path(conn, :create), sub: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Sub"
    end
  end
  ...
  describe "edit sub" do
    ...
    test "renders form for editing chosen sub", %{conn: conn, sub: sub} do
      conn = get(conn, Routes.sub_path(conn, :edit, sub))
  ...
  describe "update sub" do
    ...
    test "redirects when data is valid", %{conn: conn, sub: sub} do
      conn = put(conn, Routes.sub_path(conn, :update, sub), sub: @update_attrs)
      assert redirected_to(conn) == Routes.sub_path(conn, :show, sub)

      conn = get(conn, Routes.sub_path(conn, :show, sub))
      assert html_response(conn, 200)
    end

    test "renders errors when data is invalid", %{conn: conn, sub: sub} do
      conn = put(conn, Routes.sub_path(conn, :update, sub), sub: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Sub"
    end
  end
  ...
  describe "delete sub" do
    ...
    test "deletes chosen sub", %{conn: conn, sub: sub} do
      conn = delete(conn, Routes.sub_path(conn, :delete, sub))
      assert redirected_to(conn) == Routes.sub_path(conn, :index)
      assert_error_sent 404, fn ->
        get(conn, Routes.sub_path(conn, :show, sub))
      end
    end
  end
